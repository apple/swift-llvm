...
// See rdar://45939472, this is used to restrict stack coloring in the caller.
static void stripLifetimeMarkers(Function &F) {
  SmallVector<Instruction *, 8> EraseMe;
  for (Instruction &I : instructions(F)) {
    ...
    if (Intrin->getIntrinsicID() == Intrinsic::lifetime_start ||
      ...
      EraseMe.push_back(Intrin);
  }
  for (Instruction *I : EraseMe)
    I->eraseFromParent();
}
...
bool HotColdSplitting::outlineColdRegions(Function &F, ProfileSummaryInfo &PSI,
  ...
  while (!OutliningWorklist.empty()) {
    ...
    do {
      ...
      if (Outlined) {
        if (OutlinedFunctionID == 1)
          stripLifetimeMarkers(F);
